...
export default function CrudModal({ title, fields, initialValues, onClose, onSubmit, pk, pkEditable, isEdit }) {
  ...
  const handleChange = (f, val) => {
    setValues((v) => {
      ...
      // auto-fill sibling fields from the selected option's row data (parent -> child)
      if (f.autofill) {
      ...
      // keep linked sibling selects in sync (e.g. QR Code No / Label / Image all
      // describing the same underlying row) — whichever one changes, look up the
      // matching row via this field's own column, then set every other field in
      // the group using ITS OWN optionsValue column from that same row.
      if (f.linkGroup) {
        const rows = dynamicRawData[f.name] || [];
        const selectedRow = rows.find((r) => String(r[f.optionsValue]) === String(val));
        fields.forEach((g) => {
          if (g.linkGroup === f.linkGroup && g.name !== f.name) {
            next[g.name] = selectedRow ? (selectedRow[g.optionsValue] ?? '') : '';
          }
        });
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // for fields with resolveColumn, submit the real underlying column value
      // (looked up from the matching row) instead of the lightweight display value
      const payload = {};
      fields.forEach((f) => {
        if (f.uiOnly) return;
        if (f.resolveColumn) {
          const rows = dynamicRawData[f.name] || [];
          const row = rows.find((r) => String(r[f.optionsValue]) === String(values[f.name]));
          payload[f.name] = row ? row[f.resolveColumn] : values[f.name];
        } else {
    ...
    return (
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <input
          ...
          style={{ flex: 1 }}
        />
        <QRScannerButton onScan={(text) => handleChange(f, text)} />
      </div>
    );
  }
  return (
