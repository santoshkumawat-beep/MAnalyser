    ...
    dynamicFields.forEach(async (f) => {
      try {
        ...
        const opts = res.data.map((row) => ({
          value: row[f.optionsValue],
          label: row[f.optionsLabel] || row[f.optionsValue],
        }));
        setDynamicOptions((prev) => ({ ...prev, [f.name]: opts }));
      } catch {
        setDynamicOptions((prev) => ({ ...prev, [f.name]: [] }));
      }
    });
            ...
            <div className="form-grid">
              {fields.map((f) => (
                <div key={f.name} className={f.type === 'textarea' ? 'form-field-full' : ''}>
                  ...
                  {renderInput(f, values, handleChange, isEdit, pk, dynamicOptions)}
                </div>
              ))}
            </div>
          </div>
...
function renderInput(f, values, handleChange, isEdit, pk, dynamicOptions) {
  const isPkLocked = isEdit && f.name === pk; // don't allow editing the primary key once created

  if (f.type === 'select') {
    const opts = f.optionsEndpoint
