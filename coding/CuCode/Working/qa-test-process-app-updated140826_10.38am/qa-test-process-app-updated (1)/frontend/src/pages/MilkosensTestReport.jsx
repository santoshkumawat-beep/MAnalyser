...
// Admin Report screen for the MilkoSens Component Test Process.
//
// Visible only to:
...
// hands back.
export default function MilkosensTestReport() {
  ...
  useEffect(() => {
    api.get('/process/milkosens-test/admin-report/meta')
      .then((r) => {
        setMeta(r.data);
  ...
  const runReport = async (overrides = {}) => {
    ...
    try {
      ...
      const { data } = await api.get('/process/milkosens-test/admin-report', { params });
      setRows(data.rows);
  ...
  if (metaError) {
    return (
      <div className="card">
        <div className="card-header"><h3>MilkoSens Test Report</h3></div>
    ...
    <div>
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card-header">
          <h3>MilkoSens Test Report{meta?.role === 'superAdmin' ? ' — Super Admin (all Internal groups)' : ''}</h3>
        </div>
